...
const BrandItemCard = ({ brand }) => {
    ...
    return (
        <div className="w-full flex flex-col xs:flex-row items-start xs:items-center p-2 xs:p-4 gap-2">
            <div className="w-full xs:w-11/12 grid grid-cols-1 xs:grid-cols-12 gap-2 xs:gap-4">
                <div className="col-span-1 xs:col-span-5 flex flex-col xs:flex-row items-center xs:items-center gap-2 xs:gap-3">
                    ...
                    />
                    <h3 className="text-base xs:text-lg font-semibold text-center xs:text-left break-words">{brand?.name}</h3>
                </div>
                <div className="col-span-1 xs:col-span-7 mt-2 xs:mt-0">
                    <p className="text-xs xs:text-sm text-gray-600 break-words">{brand?.description}</p>
                </div>
            </div>
